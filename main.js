...
// TransformControls 操作中は OrbitControls を無効化（競合回避）
...
canvas.addEventListener('drop', (e) => {
    ...
    const intersects = getIntersects(e, [floor]);
...
canvas.addEventListener('click', (e) => {
    ...
    if (intersects.length > 0) {
        ...
        if (transControls.object === targetModel) {
            ...
            switch (transControls.getMode()) {
                case 'translate':
                    ...
                    break;
                case 'rotate':
                    transControls.detach(); // 回転の次は選択解除
                    break;
            }
        } else {
            // 別のモデルをクリックした場合、それにアタッチする
        ...
        // モデル以外（何もない空間）をクリックした場合
        // ただし、TransformControlsのギズモ自体をクリックした場合は何もしない
        gizmo.visible = false;
